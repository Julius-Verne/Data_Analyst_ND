...
value in the dataset, 214, that belongs to the article of Jesus Christ, who is 
...
> # Conclusions: Gender Exploration

> First, there were some differences 
in occupations between men and women. Most notable men worked in institutions 
while women worked in the Arts.
...
> After applying log10 to the variable we see the changes are minimum, there is 
no clear relationship between a HPI and page views.

> I double checked by using the pearson correlation index, which is lower than 
it was before. Which is already pretty close to 0 or "No relation".
...
average views. Nevertheless 0.4 is still too low to consider a relationship 
between the two.
...
```{r fig.height=15, message= FALSE, echo=FALSE }
